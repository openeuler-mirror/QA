...
摘要：依据测试要求，对openmp进行自编译、安装卸载验证及功能测试。
...
openmp特性，共执行262个测试用例，代码覆盖率90%。覆盖特性的基本功能，自编译以及安装卸载，不涉及命令验证和服务验证。未发现问题，所有测试点全部通过
