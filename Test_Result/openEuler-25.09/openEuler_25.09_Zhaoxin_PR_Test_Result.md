...
| 日期       | 修订   版本 | 修改描述 | 作者    |
| ---------- | ----------- | -------- | ------- |
| 2025/11/14 | 1.0.0       | 初稿     | Lyle Li |

关键词： Zhaoxin 

摘要：本文主要介绍在openEuler 25.09版本上对Zhaoxin OLK-6.6补丁包进行测试的相关情况，包括测试环境、测试过程与测试结果。

# 1     特性概述

该测试报告叙述了在openEuler 25.09版本上，针对Zhaoxin处理器进行Zhaoxin OLK-6.6补丁测试的相关情况。
...
| 版本名称        | 测试起始时间 | 测试结束时间 |
| --------------- | ------------ | ------------ |
| openEuler 25.09 | 2025/11/14   | 2025/11/14   |
...
| Zhaoxin KX-7000  | 板卡：HX005EZ0-8Core-2GHz 内存： 硬盘：SATA SSD 500G       |      |

# 3     测试结论概述

## 3.1   测试整体结论

在基于openEuler 25.09版本的用于Zhaoxin CPU的操作系统内核中，针对其功能共计执行两项测试，功能正常，整体质量良好；
...
| 5    | cpufreq: ACPI: add ITMT support when CPPC enabled           |             <font color=green>■</font>             | 测试通过，未发现问题；                                       |
| 6    | ata: ahci: Add support for AHCI SGPIO Enclosure Management  | <font color=blue><font color=green>■</font></font> | 测试通过，未发现问题；读写触发active LED闪烁测试、fail模式测试、identify模式测试均通过。 |
| 7    | i2c: smbus: Add support for Zhaoxin SMBUS controller        | <font color=blue><font color=green>■</font></font> | 测试通过，未发现问题。                                       |
| 8    | efi:cper: Add Zhaoxin/Centaur ZDI/ZPI error decode          | <font color=blue><font color=green>■</font></font> | 测试通过，未发现问题。                                       |
