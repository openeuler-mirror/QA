...
| 2026/3/2 | 1.0.4 | update RISC-V | jean9823 |
...
| SpacemiT K3-CoM260 | 进迭时空 K3 | OS集成测试 |
...
| 59 | 集成openGauss 6.0.0 LTS企业版 | <font color=green> █  </font> | <font color=green>█</font>   | <font color=green>█</font> | NA |重点验证openGauss 6.0.0数据库系统工具、SQL功能、数据库升级、兼容B库模块等功能测试 |
...
|8|[openssl：Backport 主线 RISC-V 架构的 SHA-2 汇编优化](https://gitee.com/openeuler/QA/pulls/1250/files)| RVA23: 环境变量 `OPENSSL_riscvcap` 设置为 `rv64gc_zbb`，强制启用 RISC-V 64 下的 Zbb 优化，执行 openssl 编译过程自带的 make test 测试，测试可以通过；RVA20：不作环境变量指定执行 openssl 编译过程自带的 make test 测试，测试可以通过  | | |NA|<font color=green>█</font>  | <font color=green>█</font> | NA | |
...
|25|[UB lib & tool：灵衢基础工具和库](https://atomgit.com/openeuler/QA/pull/1315)| 基于 openEuler-24.03-LTS-SP3 RC6、RC7、RC10版本的灵衢基础ubutils & ubctl工具和libCDMA & UMMU-DK库，针对其功能完成所有转测的需求的用例验收，执行1409，通过1376，通过率97.7%，发现3个问题单，整体质量较好。  | | | <font color=green>█</font> | <font>NA</font> | <font>NA</font> | NA ||
|26|[openssl：Backport 主线 RISC-V 架构的 MD5 汇编优化](https://atomgit.com/openeuler/QA/pull/1312)|RVA23：环境变量 `OPENSSL_riscvcap` 设置为 `rv64gc_zbb`，强制启用 RISC-V 64 下的 Zbb 优化，执行 openssl 编译过程自带的 make test 测试，2890 个测试案例可以通过；RVA20：不作环境变量指定执行 openssl 编译过程自带的 make test 测试，2890 个测试案例可以通过| | | <font>NA</font> | <font color=green>█</font> | <font color=green>█</font> | NA ||
...
|34|[引入IOInflight QoS控制器，降低混部场景在线业务干扰率](https://atomgit.com/openeuler/QA/pull/1316)| 共计执行21个用例，主要覆盖了功能测试、可靠性测试。其中功能测试用例通过；经过3*24的可靠性测试，用例全部测试通过，整体质量良好。  | | | <font color=green>█</font> | <font>NA</font> | <font>NA</font> | NA ||
...
|38|[ModelFS提升推理服务启动速度](https://atomgit.com/openeuler/QA/pull/1316)| ModelFS特性，共计执行119个用例，主要覆盖了功能测试和可靠性测试，经过3*24h的可靠性测试，发现问题已解决，回归通过，无遗留风险，整体质量良好。  | | | <font color=green>█</font>| <font>NA</font> | <font>NA</font> | NA ||
...
| riscv64 | 进迭时空SpacemiT | K3 | 进迭时空SpacemiT | pass |
...
|--------------|-------------|-------------------------|-----------------------------------|-----------------------------------|-----------------------------------|-----------------------------------|
| OS基础性能   | 进程调度子系统，内存管理子系统、进程通信子系统、系统调用、锁性能、文件子系统、网络子系统。 | 参考版本相应指标基线 | 与基线版本（24.03-LTS）对比不劣化5%以内目标已达成 | 与基线版本（24.03-LTS）对比不劣化5%以内目标已达成 | 该版本是 RVA23 标准第一版，故以此版本测试结果作为指标基线 |与基线版本（24.03-LTS）对比不劣化5%以内目标已达成 |
