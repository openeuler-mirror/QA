...
| 日期 | 修订   版本 | 修改描述 | 作者 |
| ---- | ----------- | -------- | ---- |
|   2025-12-15   |      openamdc_v1.1       |    openamdc性能优化与问题修复     |  changzhi    |
|      |             |          |      |
...
| 版本名称 | 测试起始时间 | 测试结束时间 |
| -------- | ------------ | ------------ |
|  openEuler-24.03-LTS-SP3-RC5  |  2025-11-28     |   2025-12-15    |
|  openEuler-24.03-LTS-SP3-RC6   |  2025-11-28     |   2025-12-15    |
...
| 功能测试 | 继承特性测试 | 功能运行正常     |
| 功能测试 | 新增特性测试 | 功能符合需求     |
| 兼容性测试 |  跨OS版本测试     | 在 24.03sp2 和 25.03 版本上运行正常     |
...
|    序号    | 问题简述 | 优先级 | 当前状态 |
| ------ | -------- | ---- | ---- |
| 1 |     Lua 脚本可能导致远程代码执行漏洞      |   主要   |  已验收    |
| 2 |     Lua 脚本可能引发整数溢出及潜在远程代码执行漏洞     |   主要  |   已验收  |
| 3 |     Lua 脚本可在其他用户上下文环境中执行     |   主要  |   已验收  |
| 4 |     Lua 越界读取漏洞     |   主要  |   已验收  |
| 5 |     HyperLogLog 在条目达到 2GB 及以上时可能出现崩溃问题  |   主要  |   已验收  |
| 6 |     主从模式服务均开启多线程，压测服务器阻塞，主节点缓冲区异常，从节点响应慢   |   主要  |   已验收  |
...
本次兼容性测试覆盖了以下主要方面：
操作系统兼容性：openEuler 24.03sp2 与 openEuler 25.03 上测试，未发现兼容性问题。
