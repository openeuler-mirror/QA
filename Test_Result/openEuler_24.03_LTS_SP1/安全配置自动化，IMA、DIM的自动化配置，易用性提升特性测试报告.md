...
1、目前sec_conf支持可配置的安全机制为IMA、DIM、secure boot、modsign，其他安全特性暂未支持。
2、IMA度量和评估只支持sec_conf中指定的模式，其他策略与模式暂未支持。
...
| 序号 | 测试内容 | 测试结论 |
| ------- | ------- | -------- |
|    1     |  用户生成配置脚本后，可直接执行脚本，无需手动处理，即可完成安全特性配置，减少了用户配置错误，可能导致系统无法启动或无法正常运行的情况。     |    ■      |
|    2     |  用户生成检查脚本后，执行脚本，即可以检查环境是否支持开启安全特性，也可以检查配置脚本的执行结果是否符合预期，减少了手动验证的繁琐步骤和潜在的人为疏忽，显著提高了配置和验证过程的准确性和效率。     |    ■      |
