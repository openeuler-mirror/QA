...
| 日期 | 修订   版本 | 修改描述 | 作者 |
| ---- | ----------- | -------- | ---- |
|  2023/03/28  |  v1.0           |测试报告初版          |王鹏      |
| 2023/03/28 | v1.1 |增加问题单总结 |王鹏 |
...
DDE特性测试分别在RC1/RC2/RC3/RC4四轮验证，同时验证ARM版本和X86版本。

| 版本名称 | 测试起始时间 | 测试结束时间 |
| -------- | ------------ | ------------ |
| openEuler 23.03 RC1 |  2023-02-25  | 2023-02-28 |
| openEuler 23.03 RC2 |      2023-03-03      |    2022-03-08    |
...
DDE 特性总共进行两轮测试，合计执行用例343条，主要覆盖了基础组件、预装应用核心功能、新增特性基础功能以及基本UI测试。

第一轮发现问题1个，回归通过0个，遗留问题1个。

第二轮发现问题1个，回归通过1个，遗留问题1个。

第三轮发现问题0个，回归通过1个，遗留问题0个。

第三轮发现问题0个，回归通过0个，遗留问题0个。
...
| -------- | ------------------------------------ | -------- | -------------------------------- | -------- |
| I6KP18   | driver-installer.service服务启动失败 | 次要     | 管理显卡服务，不影响其他功能使用 | 挂起     |

### 3.3.2 问题统计

| 问题单号 | 问题描述           | 问题级别 | 问题影响                  | 当前状态 |
| -------- | ------------------ | -------- | ------------------------- | -------- |
| I6IGHP   | 转测源中无dde组件  | 严重     | DDE组件安装失败，阻塞测试 | 已验收   |
| I6KZHF   | 输入法无法切换中文 | 次要     | 无法输入中文              | 已验收   |
...
| -------- | ---------- | ------------ | ------------ |
| openEuler 23.03 RC1 ARM+X86 |  1  |    0PASS + 1FAIL    |        1   |
|openEuler 23.03 RC2  ARM+X86  |230    |  229PASS + 1FAIL  |       1       |
|openEuler 23.03 RC3  ARM+X86 |56 | 56PASS + 0FAIL | 0 |
|openEuler 23.03 RC4  ARM+X86 |56 | 56PASS + 0FAIL | 0 |
