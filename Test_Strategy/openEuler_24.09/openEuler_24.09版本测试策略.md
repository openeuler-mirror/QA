...
| iSulad支持CRI v1.29  | 继承已有测试能力，验证iSulad容器引擎对CRI v1.29通信协议的支持 | √ | √ |  |  |  |
| iSulad支持CDI        | 继承已有测试能力，验证iSulad对CDI的支持，支持用户加载符合 CDI 标准的设备 | √ | √ |  |  |  |
| iSulad支持cgroup v2    | 继承已有测试能力，验证iSulad对 cgroup v2 的支持，能够让 Kubernetes 委派更安全的 cgroup 子树给容器，支持跨多个资源的增强资源分配管理和隔离 | √ | √ |  |  |  |
