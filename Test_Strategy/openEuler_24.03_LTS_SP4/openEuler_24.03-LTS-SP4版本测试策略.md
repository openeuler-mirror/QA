...
| 2026-04-28 | 1.0.1 | | add RISC-V | jean9823 |
...
| ------------ | -------------------------------- | -------------------------------- |-------------------------------- | ---- | ---- | ------ | --------- | --------- |
|1|[增加可信计算资源分发服务组件](https://gitcode.com/openeuler/release-management/issues/2366)|[详细测试策略文件]()| |√ |　 √ | NA | × |× |
|2|[远程证明支持DICE](https://gitcode.com/openeuler/release-management/issues/2434)|[详细测试策略文件]()| | √ |　  | NA | × |× |
|3|[海光ccp驱动升级，支持sm4-xts和sm4-gcm](https://gitcode.com/openeuler/release-management/issues/2441)|[详细测试策略文件]()| |× |　 √  | NA | × |× |
|4|[新增海光CIS指令集sm3,sm4驱动](https://gitcode.com/openeuler/release-management/issues/2442)|[详细测试策略文件]()| |× |　√ | NA | × |× |
|5|[支持树莓派](https://gitcode.com/openeuler/release-management/issues/2448)|[详细测试策略文件]()| | √ |　 × | NA | × |× |
...
| ----------------------------------------------------- | ------------------------------------------------------------ | ---- | ---- | ------ | ------ |--------- |
| 支持DDE桌面                                           | 继承已有测试能力，关注DDE桌面系统的安装和基本功能            | √    | √    | NA    | √     |√   |
| 支持UKUI桌面                                          | 继承已有测试能力，关注UKUI桌面系统的安装和基本功能           | √    | √    | NA    | √     | √   |
| 支持Kiran桌面                                         | 增强特性新增测试，其余继承已有测试能力，关注kiran桌面系统的安装和基本功能 | √    | √    | NA    | √     |  √   |
| 安装部署                                              | 继承已有测试能力，覆盖裸机/虚机场景下，通过光盘/USB/PXE三种安装方式，覆盖最小化/虚拟化/服务器三种模式的安装部署 | √    | √    | NA   | √     |√   |
| 内核                                              | 继承已有测试能力，重点关注本次版本发布特性涉及内核配置参数修改后，是否对原有内核功能有影响；采用开源测试套LTP/mmtest等进行内核基本功能的测试保障；通过开源性能测试工具对内核性能进行验证，保证性能基线与LTS基本持平，波动范围小于5%以内 | √    | √    | NA   | √     |√   |
| 容器(isula/docker/安全容器/系统容器/镜像)             | 继承已有测试能力，重点关注本次容器领域相关软件包升级后，容器引擎原有功能完整性和有效性，需覆盖isula、docker两个引擎；分别验证安全容器、系统容器和普通容器场景下基本功能验证；另外需要对发布的openEuler容器镜像进行基本的使用验证 | √    | √    | NA    | √      |√   |
| 虚拟化                                                | 继承已有测试能力，重点关注回合新特性后，新版本上虚拟化相关组件的基本功能 | √    | √    | NA   | √   |√   |
| 支持A-Tune                                            | 继承已有测试能力，本次无新增合入，重点关注继承功能验证，如服务/配置检查等。 | √    | √    | NA | √   |×    |
| 支持secPaver                                           | 继承已有测试能力，关注secPave特性的基本功能和服务的稳定性    | √    | √    | NA  | √    |√   |
| 支持secGear                                           | 继承已有测试能力，关注secGear特性的功能完整性，重点关注上次新增合入远程证明框架的功能完整性和准确性。               | √    | √    | NA   | ×    |×    |
| 支持kubeOS                                            | 继承已有测试能力，重点验证kubeOS提供的镜像制作工具和制作出来镜像在K8S集群场景下的双区升级的能力 | √    | √    | NA   | ×    |×    |
| 支持etmem内存分级扩展                                 | 继承已有测试能力，重点验证特性的基本功能和稳定性             | √    | √    | NA   | ×    |×    |
| 支持用户态协议栈gazelle                               | 继承已有测试能力，重点关注gazelle高性能用户态协议栈功能      | √    | √    | NA | √  |×    |
| 支持国密算法                                          | 继承已有测试能力，验证openEuler操作系统对关键安全特性进行商密算法使能，并为上层应用提供商密算法库、证书、安全传输协议等密码服务。 | √    | √    | NA   | √    |√   |
| 支持pod带宽管理oncn-bwm                               | 继承已有测试能力，验证命令行接口，带宽管理功能场景，并发、异常流程、网卡故障以及ebpf程序篡改等故障注入，功能生效过程中反复使能/网卡Qos功能、反复修改cgroup优先级、反复修改在线水线、反复修改离线带宽等测试 | √    | √    | NA   | ×    |√   |
| 支持isuald                             | 继承已有测试能力，重点验证isulad长稳场景 | √    | √    | NA   | √    |√   |
| 支持A-OPS                             | 继承已有测试能力，重点关注本次新增合入容器干扰检测，微服务性能问题分钟级定位定界场景| √    | √    | NA   | ×    |×    |
| 支持系统运维套件x-diagnosis                           | 继承已有测试能力，覆盖x-diagnosis的问题定位工具集、系统巡检、ftrace增强等功能 | √    | √    | NA | ×  |×    |
| 支持自动化热升级组件nvwa                              | 继承已有测试能力，覆盖内核热升级管理能力：内核热升级命令行、保持业务的配置、升级状态查询、热升级特性开关等 | √    | × | NA   | ×    |√   |
| 支持DPU直连聚合特性dpu-utilities                                   | 继承已有测试能力，验证DPU支持将管理面进程无感卸载到DPU，搭配网络、存储、安全等的卸载，释放主机计算资源 | √    | √    | NA   | ×    |× |
| 支持系统热修复组件syscare                             | 继承已有测试能力，验证热补丁服务管理工具syscare在补丁管理、补丁制作等能力，重点关注新增合入栈检测，容器化能力 | √    | √    | NA    | √     |× |
|iSula容器镜像构建工具isula-build  | 验证通过Dockerfile文件快速构建容器镜像，并支持镜像的查询、删除、登录、退出等功能 | √    | √    | NA | √   |×    |
| 支持进程完整性防护特性DIM                                | 继承已有测试能力，验证DIM动态完整性度量特性支持在内核模块代码段等关键内存数据的度量能力 | √    | √    | NA | √   |× |
| 支持入侵检测框架secDetector                           | 继承已有测试能力，验证secDetector 入侵检测系统支持检测能力、响应能力和服务能力等 | √    | √    | NA | ×  |×     |
| isocut镜像裁剪                                        | 继承已有测试能力，验证基于openEuler发布的标准ISO镜像进行最小系统定制裁剪，定制安装过程中支持按需裁剪RPM包 | √    | √    | NA  | √    |×     |
| 支持devmaster组件                                     | 继承已有测试能力，验证devmaster的安装部署、进程配置、客户端工具等使用场景 | √    | √    | NA  | √   |×     |
| 支持TPCM特性                                          | 继承已有测试用例，验证openEuler支持TPCM能力，覆盖shim和grub支持国密算法度量、上报度量信息到BMC、接收BMC控制命令等 | √    | × | NA   | ×    |×     |
| 支持sysMaster组件                                     | 继承已有测试能力，验证sysMaster组件支持进程、容器和虚拟机的统一管理能力，覆盖创建单元配置文件、管理单元服务等场景 | √    | √    | NA    | √     |× |
| 支持sysmonitor特性                                    | 继承已有测试能力，验证sysmonitor监控OS系统运行过程中的异常，并将监控到的异常记录到系统日志的能力，覆盖文件监控、磁盘分区监控、网卡监控、cpu监控等场景 | √    | √    | NA   | √    |√     |
| 混合部署rubik                                         | 继承已有测试能力，覆盖rubik容器调度在业务混合部署的场景下，根据QoS分级，对资源进行合理调度，保障在线业务QoS基本无变化 | √    | √    | NA   | ×    |√     |
| 支持IMA自定义证书                             | 继承已有测试能力，验证rpm构建时，使用第三方证书对rpm摘要列表进行签名，内核导入第三方证书后，IMA摘要列表功能正常，以及xfs文件系统下，正常开启IMA摘要列表评估模式。| √    | √    | NA  | ×    |×    |
| 支持IMA virtCCA                             |继承已有测试能力，验证可信根为virtcca时，IMA度量扩展日志可正常扩展到可信根，以及存在全0度量日志，系统不会crash等； | √    | √    | NA   | ×    |×    |
| 安全启动                            | 继承已有测试能力，验证bios导入证书后，正常开启安全启动，以及防回滚功能开启后，无法进行版本降级操作等；| √    | √    | NA   | ×    |×    |
| Kmesh                              | 继承已有测试能力，验证mdacore使能\去使能\查询功能，k8s场景的fortio网格加速测试、非容器场景的tcp网格加速功能，kmesh支持pod粒度/namespace粒度流量治理功能等。| √    | √    | NA   | ×    |×    |
| openEuler安全配置规范框架设计及核心内容构建                             |继承已有测试能力，验证安全配置构建工程可以正常构建，安全配置指导内容正确，具有指导性 | √    | √    | NA  | √   |√   |
| oemaker                              |继承已有测试能力，在构建工程中保证oemaker功能正常 | √    | √    | NA   | √    |√   |
| openssl                              |继承已有测试能力，验证相比关闭指令集加速开关，sm4算法的加解密速度在默认打开情况下提升40倍以上 | √    | √    | NA   | √    |√    |
| 编译器(gcc/jdk)                                       | 继承已有测试能力，基于开源测试套对gcc和jdk相关功能进行验证   | √    | √    | NA    | √     |√    |
| 支持HA软件                                            | 继承已有测试能力，重点关注HA软件的安装部署、基本功能和可靠性 | √    | √    | NA  | √   | ×    |
| 支持KubeSphere                                        | 继承已有测试能力，关注kubeSphere的安装部署和针对容器应用的基本自动化运维能力 | √    | √    | NA   | ×    |×    |
| 支持openstack Train 和 Wallaby                        | 继承已有测试能力，验证T和W版本的安装部署及各个组件提供的基本功能 | √    | √    | NA   | ×    |√    |
| 支持智能运维助手                                     | 继承已有测试能力，关注智能定位（异常检测、故障诊断）功能、可靠性 | √    | √    | NA   | ×    |×    |
| 支持k3s                                               | 继承已有测试能力，验证k3s软件的部署测试过程                  | √    | √    | NA   | ×    |×    |
| migration-tools增加图形化迁移openeuler功能            | 继承已有测试能力，验证migration-tools图形化迁移工具支持其他操作系统快速、平滑、稳定且安全地迁移至 openEuler 系操作系统 | √    | √    | NA   | ×    |√    |
| 发布Nestos-kubernetes-deployer                        | 继承已有测试能力，覆盖在NestOS上部署，升级和维护kubernetes集群功能正常 | √    | √    | NA   | ×    |×    |
| 支持NestOS                                            | 继承已有测试能力，关注NestOS各项特性：ignition自定义配置、nestos-installer安装、zincati自动升级、rpm-ostree原子化更新、双系统分区验证 | √    | √    | NA   | ×    |×    |
| 发布PilotGo及其插件特性新版本                         | 继承已有测试能力，验证PilotGo支持 topo 图的展示和智能调优能力 | √    | √    | NA   | √    |×    |
| 社区签名体系建立                                      | 继承已有测试能力，验证安装 openEuler 镜像后，开启安全启动、内核模块校验、IMA、RPM 校验等按机制，在系统启动和运行阶段使能相应的签名验证功能，保障系统组件的真实性和完整性。 | √    | √    | NA    | ×     |×    |
| 支持GreatSQL                                | 继承已有测试能力验证openEuler支持高可用、高性能、高安全、高兼容的GreatSQL开源数据库 | √    | √    | NA  | √   |√    |
| 支持RISC-V                                            | 继承已有测试能力，关注openEuler版本在RISV-V处理器上的可安装和可使用性 | √    | √    | NA    | √     |×    |
| 为 RISC-V 架构引入 Penglai TEE 支持                   | 继承已有测试能力，验证openEuler操作系统在RISC-V 架构上对可扩展 TEE的支持，使能高安全性要求的应用场景：如安全通信、密码鉴权等 | ×    | ×    | NA  | √   |×    |
| virtCCA机密虚机特性合入   |  继承已有测试能力，重点验证机密虚机的基本功能、安全、兼容性以及虚拟机注入故障/宿主机注入故障/老化测试/并发测试的可靠性测试   | √    | × | NA | × |×    |
| 增加YouQu自动化测试平台支持   |  验证YouQu自动化测试平台部署正常，并在DDE环境上执行自动化测试   | √    | √    |   NA  |   √   |×    |
| 增加 utsudo 支持    |  继承已有测试能力，验证utsudo基础命令使用正常   | √    | √    |   NA   |   √    |√    |
| 增加 utshell支持   |   继承已有测试能力，验证utshell基础命令使用正常  | √    | √    |   NA   |   √    |√    |
| 海光CSV3支持（支持主机创建CSV3虚拟机，支持虚拟机中运行内核）   |  重点验证CSV3虚拟机的不同启动方式，覆盖功能、稳定性、兼容性测试   | × | √    | NA | × |×    |
| 集成openGauss 6.0.0 LTS企业版    |   重点验证openGauss 6.0.0数据库系统工具、SQL功能、数据库升级、兼容B库模块等功能测试   | √    | √    | NA | √ |×    |
| LLVM多版本实现    |  继承已有测试能力，验证LLVM多版本下，全量版本构建正常、LLVM多版本包能够正常工作和使用   | √    | √    | NA | √ |√   |
| 新增密码套件openHiTLS     |  继承已有测试能力，重点验证openHiTLS密码算法、密码协议和证书的功能测试   | √    | √    | NA | × |×    |
| 支持oeaware    |  继承已有测试能力，重点验证oeaware插件框架以及采集、感知等插件，主要覆盖了服务测试、客户端测试、框架测试、可靠性测试、安全测试等测试内容   | √    | √   | NA | × |×    |
| AI集群慢节点快速发现 Add Fail-slow Detection   |   继承已有测试能力，重点验证组内多节点/多卡空间维度对比，输出慢节点/慢卡的检测精度  | √    | √    |  NA   |  ×    |×    |
| RPM国密签名支持  |  重点验证异常参数解析&异常配置文件接口功能，验证gpg支持生成国密公私钥、生成国密证书，、国密签名和验签，rpm支持国密算法签名和验签等测试内容   | √    | √    |  NA  |  √   |√   |
| 鲲鹏KAE加速器驱动安装包合入  |  继承已有测试能力，验证KAE加解密加速SSL/TLS应用和使用KAEzip进行数据压缩    | √    | × | NA | × | × |
| Add Intel QAT packages support   |  继承已有测试能力，重点验证intel qat相关软件包的功能和性能   | × |   √  | NA | × | × |
| Add OpenVINO packages native support  |   继承已有测试能力，验证OpenVINO框架上sample code编译运行成功和OpenVINO的推理功能   | √    | √    | NA | × | × |
| Add oneAPI low level native support  |   继承已有测试能力，重点验证oneAPI对于sample code的编译运行，大模型基于oneAPI加速框架的正确支持以及相应软件包的功能测试   | √    | √    | NA | × | × |
| 版本引入ACPO包   |  继承已有测试能力，重点验证使能ACPO、使用ACPO进行模型训练和推理，覆盖功能、性能和可靠性测试内容   | √    | √    |   NA   |   ×   | × |
| 内核TCP/IP协议栈支持CAQM拥塞     |    继承已有测试能力，验证CAQM拥塞控制算法使能后标准功能和性能  | √    | √    | NA | × | × |
| openEuler安全配置基线检测工具      |    继承已有测试能力，使用Linux系统安全检查工具secureguardian，通过执行一系列的安全检查脚本, 查看生成的安全报告，评估系统的安全性是否存在风险  | √    | √    | NA | √ |√ |
| 支持树莓派     |    继承已有测试能力，对树莓派镜像进行内核版本检查，安装、基本功能、管理工具、硬件兼容性等测试  | √    | ×    | NA | × |× |
|OVMF_CODE.fd支持CSV1/2/3开箱即用 | 继承已有测试能力，主要包括功能测试，稳定性测试，压力测试|  ×    | √     | NA | × |× |
|OVMF.fd支持CSV1/2/3开箱即用 | 继承已有测试能力，主要包括功能测试,稳定性测试|  ×    | √     | NA | × |× |
|新的psp/ccp device id支持 | 继承已有测试能力，主要覆盖海光可信功能验证|  ×    | √     | NA | × |× |
|可信功能内核驱动剥离sev依赖 |继承已有测试能力，主要覆盖海光可信功能验证 |  ×    | √     | NA | × |× |
| vTKM 性能优化|继承已有测试能力，主要覆盖普通虚拟机和CSV虚拟机下的TKM功能验证，性能测试验证,内部密钥的SM2签名、SM2解密、SM4。 |  ×    | √     | NA | × |× |
| 基于通信算子的低开销高精度慢节点检测 |继承已有测试能力，覆盖功能、长稳、资料测试，功能覆盖模型数据timeline图转化，数据采集开销验证，分组验证，慢节点分析验证|  √    | √     | NA | × |× |
| FUSE passthrough支持|继承已有测试能力，主要覆盖功能测试和性能测试 | √    | √     | NA | × |× |
| 基于eBPF等技术实现AI作业进程Stack Mergeing，支持典型内存故障定位| 继承已有测试能力，主要覆盖功能测试，功能覆盖模型内存timeline图转化、内存堆栈转化，可以在网页上正确显示火焰图和时间线 | √    | ×     | NA | × |× |
| 虚拟化场景支持vKAE直通设备热迁移 | 继承已有测试能力，覆盖功能、性能、资料测试，功能覆盖vKAE直通虚拟机热迁移验证 | √    | ×     | NA | × |× |
| secGear支持机密虚机基于UEFI启动方式的报告生成及验证 | 继承已有测试能力，覆盖功能测试和可靠性测试 | √    | ×     | NA | × |× |
| 支持可信计算远程证明服务组件 |继承已有测试能力，涉及基础冒烟测试和ltp测试| √    | √     | NA | × |× |
| 支持Kuasar机密容器镜像加解密| 继承已有测试能力，覆盖功能、可靠性、资料测试。| √    | ×     | NA | × |× |
| 支持众核高密容器级资源隔离技术 | 继承已有测试能力，测试众核调度对计算密集型与IO密集型业务服务QPS的影响，在部署密度提升超过10%的情况下业务性能良好无劣化。 | √    | √     | NA | × |× |
|  snappy库：FindMatchLength支持RISC-V架构的优化 | 继承已有测试能力，主要覆盖功能测试，验证引入snappy优化后，snappy能够正常解压缩 |     |      | NA | √  |  |
| lz4库：提高riscv架构的压缩速度 |继承已有测试能力，主要覆盖功能测试、性能测试、兼容性测试和稳定性测试，重点验证优化后的性能提升效果  |   |      | NA | √    |  |
| openssl库：RISC-V架构的系列优化-SM2优化 | 继承已有测试能力，使用官方自带测试用例，运行make test涵盖特性的所有类型验证测试；使用openssl speed工具测试加入特性前后的sm2/rsa2048/aes-128-cbc性能 |    |      | NA | √  | |
| virtcca: 支持virtCCA机密虚机热迁移 | 继承已有测试能力，主要覆盖功能测试、可靠性测试、性能测试，功能重点覆盖不同虚机规格、数量热迁移执行，业务不中断，可靠性重点覆盖迁移前可用资源检查、迁移失败源虚机恢复、并发/反复迁移稳定性、内存完整性检查等 | √  |   | NA | × | |
|  golang: Backport RVA23 | 继承已有测试能力，重点验证 Backport 后 Golang 软件包本身能够通过所有测试 |    |      | NA | √  | |
|  TSB-agent: 支持可信计算虚拟化vTPCM | 继承已有测试能力，重点验证在Host端使用 cli/api 进行虚机创建、展示、启动、停止、删除、迁移成功 | √    |      | NA | × |  |
| openssl：Backport 主线 RISC-V 架构的 SHA-2 汇编优化 | 继承已有测试能力，验证SHA-2算法核心应用场景，如数据完整性验证，数据签名基础，密码安全存储 |     |      | √ | √ | |
| URMA：URMA支持UB基础通信能力 | 继承已有测试能力，重点覆盖对用户呈现的接口测试，功能方面覆盖URMA初始化、管理面建链、数据面收发等测试，对资源不足、对端进程异常退出等可靠性场景开展测试 | √    |      | NA | × | |
| UMS：UMS支持socket接口通信 | 继承已有测试能力，重点覆盖用户使用UMS的主要场景，功能方面覆盖UMS的正常建链、逃生通道触发、建链信息打印以及配置参数写入等场景 | √    |      | NA | × | |
| DLock：支持基于UB的分布式锁 | 继承已有测试能力，覆盖对用户呈现的接口测试，功能方面覆盖DLock初始化、管理面建链、数据面收发等测试，对资源上限、进程异常退出等可靠性开展测试  | √    |      | NA | × | |
| URPC：URPC支持UB  | 继承已有测试能力，功能方面覆盖对用户呈现的接口测试、URPC初始化、管理面建链、数据面收发等功能测试，对资源不足、对端进程异常退出等可靠性场景开展测试，并开展正常使用流程的长稳压测 | √    |      | NA | × | |
| UBTurbo: HAM支持确定性热迁移能力 | 继承已有测试能力，主要覆盖基于灵衢总线的远端内存访问能力与大带宽，实现确定性时长的虚机热迁移能力 | √    |      | NA | × | |
| UBTurbo： SMAP支持内存冷热扫描和迁移能力  | 继承已有测试能力，重点验证SMAP可进行页面的冷热判断，根据冷热把页面在本地内存和远端内存之间做迁移，主要覆盖功能测试、可靠性测试和安全测试 | √    |      | NA | × | |
| UBTurbo：RMRS Agent支持内存迁移决策与执行  | 继承已有测试能力，重点覆盖RMRS Agent对用户提供的API接口，验证内存迁移决策与执行功能 | √    |      | NA | × | |
| UB Turbo： UBTurbo提供基础框架(配置读取、插件加载、日志管理、IPC通信等)能力    |   继承已有测试能力，构建UBTurbo测试插件，覆盖UBTurbo框架提供的插件注册/去注册,配置项获取以及日志记录功能   | √  |  | NA |.× | |
| UBS Engine: UBS Engine支持UB池化资源管理能力 | 继承已有测试能力，结合UBS Engine需求的交付的能力，覆盖接口/功能、场景、专项测试 | √    |      | NA | × | |
| UBS Comm: HCOM支持UB通信能力 | 继承已有测试能力，重点测试UB通信单边Read/Write，双边send以及多路径能力，基于场景侧通信能力要求，多并发大数据量发送验证  | √    |      | NA | × | |
| OBMM: 提供跨节点内存访问与一致性控制能力 | 继承已有测试能力，主要覆盖接口／功能，以及场景测试,验证OBMM提供跨节点内存访问与一致性控制能力功能正常 | √    |      | NA | × | |
| sysSentry：支持UB故障劫持能力及超节点故障上报能力 | 继承已有测试能力，重点覆盖panic/reboot场景劫持通过URMA/UVB通道上报消息答复功能；主动下电/oom通过sysSentry内部逻辑上报消息答复功能；ub内存故障上报以及故障上报后根据物理地址杀掉对应的业务进程功能 | √    |      | NA | × | |
| qemu：支持基于urma通道的内存热迁移 | 继承已有测试能力，重点验证基于urma通道的内存热迁移功能，主要覆盖功能测试、可靠性测试和性能测试  | √    |      | NA | × | |
| UBNative：灵衢虚拟化基础能力支持UBNative直通虚拟化 | 继承已有测试能力，重点验证基于虚拟机ub设备直通功能，主要覆盖功能测试、可靠性测试 | √    |      | NA | × | |
| memlink：灵衢虚拟化基础能力支持memlink大规格虚拟机、内存回收 | 继承已有测试能力，重点验证验证memlink内存回收功能， 主要覆盖功能测试、可靠性和性能测试  | √    |      | NA | × | |
|  utpam：基于Rust开发的身份认证模块  | 继承已有测试能力，重点验证utpam在openEuler版本上可正常编辑PAM配置文件设置认证策略 | √    |      | NA | × | |
| UB lib & tool：灵衢基础工具和库  | 继承已有测试能力，重点验证ubctl作为dfx工具查询公共功能配置、模块状态、统计数据和固件信息等功能  | √    |      | NA | × | |
| secScanner:操作系统安全加固组件 | 继承已有测试能力，重点验证安全加固、漏洞扫描、入侵检测在openEuler系统下正常运行，以及检测组件使用权限限制、组件日志未泄露系统敏感信息 | √    |      | NA | × | |
| VMAnalyzer:轻量级虚拟化性能监控组件 | 继承已有测试能力，重点验证虚拟机运行状况和性能采集检测、宿主机信息/虚拟化环境信息/虚机基本信息采集检测测试 | √    |      | NA | × | |
| 引入IOInflight QoS控制器，降低混部场景在线业务干扰率 | 继承已有测试能力，重点验证识别和优先处理关键任务的读写请求能力，主要覆盖功能测试和可靠性测试  | √    |      | NA | × | |
| SPDK支持vq均衡多reactor功能，降低虚拟化开销 | 继承已有测试能力，验证openEuler SPDK支持vq均衡多reactor功能，虚拟化开销小于5%，主要覆盖功能测试和性能测试 | √    |      | NA | × | |
| ModelFS提升推理服务启动速度 | 继承已有测试能力，重点验证ModelFS 语义功能和事件管理功能，以及长稳测试   |  | √    | NA | × |  |
|GMem异构融合内存提升异构资源利用率   | 继承已有测试能力，重点验证基础融合内存能力，通过内存协同，互相借用扩大内存空间提升异构资源利用率，主要覆盖功能、性能、安全和可靠性测试  | √    |      | NA | × | |
| 毕昇JDK21支持元数据压缩优化降低堆数据内存使用 | 继承已有测试能力，重点验证通过降低Java对象头占用而减少Java堆内存（Heap Memory）占用能力，主要覆盖功能和可靠性测试 | √    |      | NA | × | |
| TF原生Embedding算子图优化提高推理性能  | 继承已有测试能力，重点验证图算优化提升推理性能，主要覆盖功能测试和性能测试  | √    |      | NA | × | |
| 毕昇JDK21版本 G1、PS支持堆内存扩缩容 | 继承已有测试能力，重点验证用户可在应用运行时动态扩展Java堆内存的上限无需重启JVM的能力，主要覆盖功能、性能和可靠性测试  | √    |      | NA | × | |
| LLVM编译器提升数据库（MySQL）、压缩解压缩（lz4/zstd/xz）等性能 | 继承已有测试能力，重点验证通过编译器优化提升应用性能，主要覆盖功能、性能和可靠性测试 | √    |      | NA | × | |
| 毕昇JDK21支持JIT warmup， 提升Java启动性能 | 继承已有测试能力，重点验证开启优化特性可避免编译竞争提升应用性能，主要覆盖功能、性能和可靠性测试 | √    |      | NA | × | |
| OS灵衢组件一键安装配置，业务可感知的通信故障场景快速定位 |继承已有测试能力，UB组件可一键快速安装部署与驱动自动加载，重点验证ub-pkg-cli命令用于配置UB相关组件的配置参数与基础功能检测能力  | √    |      | NA | × | |
|  内核态操作用户态内存场景，通过容错机制避免UCE故障上报导致系统panic | 继承已有测试能力，重点验证内核在已知故障模式下可保持稳定，避免远端UB内存故障引发的内核崩溃，主要覆盖功能测试和可靠性测试 | √    |      | NA | × | |
| NPU时域切分及隔离提升利用率 | 继承已有测试能力，验证算力切分特性实现任务到异构算力的按需调度机制，主要覆盖功能、性能和可靠性测试  | √    |      | NA | × | |
| CRIU支持NPU设备dump/restore | 继承已有测试能力，验证cuiu冻结运行中的应用程序/部分组件后可通过检查点文件从冻结点恢复的能力，主要覆盖功能测试、接口测试、可靠测试和性能测试 | √    |      | NA | × | |
| AI训练场景慢卡、慢节点分钟级检测、快速定界 | 继承已有测试能力，验证在AI训练场景下，实现对XPU卡的慢卡/慢节点检测和定位，主要覆盖功能测试和可靠性测试 | √    |      | NA | × | |
|E2B Agent沙箱提供生命周期管理统一接口，支持脚本、K8S方式部署E2B集群   | 继承已有测试能力，验证E2B沙箱平台在openEuler上可一键部署，并可使用K8S进行管理部署，主要覆盖功能测试和可靠性测试 | √    |      | NA | × | |
| cgroup v2支持内存异步回收功能 | 继承已有测试能力，验证cgroup v2 内存异步回收解决当前cgroup隔离方案导致的易卡死，同步内存回收带来性能下降的痛点，主要覆盖功能测试、性能测试和可靠性测试 | √    |      | NA | × | |
| 智能Cache注入 | 继承已有测试能力，验证智能cache注入支持LLC与L2 cache stash配置，主要覆盖API接口测试、功能测试、可靠性测试和性能测试  | √    |      | NA | × | |
| sockmap加速同主机tcp流 | 继承已有测试能力，验证sockmap在socket层旁路tcp协议栈，降低延迟的作用，主要覆盖功能测试和性能测试 | √    |      | NA | × | |
| GIC超分优化，提升Nginx性能 | 继承已有测试能力，验证相同亲和组动静态迁移测试，跨亲和组动静态迁移测试，主要覆盖功能、性能和可靠性测试等 | √    |      | NA | × | |
| hisock功能增强，提升目标流量吞吐性能 | 待确认 | √    |      | NA | × | |
| sysSentry支持硬盘寿命检测和健康监控 | 继承已有测试能力，增强sysSentry故障检测能力，支持上报BMC所有硬盘告警事件，支持告警事件类型可配置化等，主要覆盖功能测试和可靠性测试 | √    |      | NA | × | |
| 内存支持zone粒度内存回收（异步模式） | 待确认 | √    |      | NA | × | |
| CCA机密虚机支持设备直通 | 测试已有测试能力，验证CCA机密虚机支持nvme盘/网卡等设备直通PF，多nvme/网卡等设备直通机密虚机  | √    |      | NA | × | |
| Go语言map SIMD加速、hashtriemap算法优化等，提升hashmap性能 | 继承已有测试能力，验证Go语言map SIMD加速、hashtriemap算法优化等效果，主要覆盖功能测试和性能测试 | √    |      | NA | × | |
|  AI图编译器TF+XLA动态batch场景并行化优化，提升性能  | 继承已有测试能力，验证XLA编译器支持动态batch和并行优化的效果，主要覆盖功能测试和性能测试 | √    |      | NA | × | |
|  LLVM实现RabitQ性能提升 | 继承已有测试能力，验证向量化增强优化和memcpy inline优化效果，主要覆盖功能测试和性能测试 | √    |      | NA | × | |
| LLVM优化降低数据库、搜推场景编译时长 | 继承已有测试能力，验证LLVM使能编译优化效果，主要覆盖功能测试和性能测试  | √    |      | NA | × | |
| 毕昇JDK GC算法、SVE/DMB指令等优化，提升大数据性能 | 继承已有测试能力，验证毕昇JDK17 GC算法、SVE/DMB指令等优化特性效果，主要覆盖功能测试和性能测试 | √    |      | NA | × | |
| 虚拟机QEMU支持UB mem D2H | 继承已有测试能力，验证A5虚拟化场景通过UB总线访问KVCache内存，主要覆盖功能测试、可靠性测试和性能测试 | √    |      | NA | × |  |
| 支持IP over URMA | 继承已有测试能力，验证IP over URMA支持智算组网TCP/IP协议通信，主要覆盖API接口测试，功能测试，并发压力测试，可靠性测试以及安全测试等 | √    |      | NA | × | |
...
| loongarch64 | 龙芯 | 3C5000 |
| loongarch64 | 龙芯 | 3C6000 |
| loongarch64 | 龙芯 | 3D6000 |
| loongarch64 | 龙芯 | 3A5000 |
| loongarch64 | 龙芯 | 3A6000 |
| riscv64 | 进迭时空SpacemiT | K3 |
