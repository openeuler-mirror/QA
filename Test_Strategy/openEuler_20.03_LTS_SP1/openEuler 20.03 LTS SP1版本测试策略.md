![](media/4e50a698862ec3974b70302cc87e88fa.png)
...
>   4.1 新增feature测试设计策略 8

>   4.2 继承feature/组件测试设计策略 9
...
本文主要描述openEuler 20.03LTS
SP1版本的总体测试策略，按照社区开发模式进行运作，结合社区release-manager团队制定的版本计划规划相应的测试活动。整体测试策略覆盖新需求、继承需求的测试分析和执行，明确各个测试周期的测试策略及出入口标准，指导后续测试活动。
...
|----------------|------------------------------------|----------|----------|--------|---------|
| 软件包生态管理 | 升级或者替代软件识别及合入版本策略 | 高       |          | 胡峰   | Opening |
| 性能指标       | 操作系统基本benchmark性能指标明确  | 高       |          | 胡峰   | Opening |
...
性能测试是针对交付件的具体性能指标，利用工具进行各类性能指标的测试。openEuler
SP版本是LTS版本的补丁版本，所以在操作系统基本benchmark各类指标上需要和LTS版本保持一致，性能数据波动需小于5%；特性类的性能，需要按照各个特性既定的指标，进行时间效率、资源消耗底噪等方面的测试验证，保证指标项与既定目标的一致。
...
openEuler
20.03LTS发布的所有可以安装的包，可以通过配置SP版本发布的repo地址，升级到本次SP版本发布的新的软件包(除新补充的包外)

本次发布SP版本在南北向兼容性方面有两个新需求，具体测试策略如下：

| 需求               | 测试方法                                                   |
|--------------------|------------------------------------------------------------|
| 支持飞腾CPU        | 通过硬件兼容性测试套件在飞腾指定服务器上完成兼容性验证     |
| 支持1822 HBA卡驱动 | 验证1822 HBA卡在openEuler 20.03LTS SP1版本上的功能正常使用 |
...
openEuler 20.03 LTS
SP1版本按照社区release-manger团队既定的版本计划，共有4轮测试，按照社区研发模式，所有的需求都在拉分支前已完成合入，因此版本的4轮测试采取3+1的测试方式，即3轮全量+1轮回归的策略。

### 测试计划

openEuler 20.03LTS
SP1版本按照社区开发模式进行运作，结合社区release-manager团队制定的版本计划规划相应的测试活动。

![](media/f2ef471f132f43f0b5e0a88e98616e5e.png)
