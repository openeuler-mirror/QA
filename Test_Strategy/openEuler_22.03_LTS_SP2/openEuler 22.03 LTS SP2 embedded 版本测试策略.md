...
| 【软实时OS】分布式软总线升级到3.2版本| sig-distributed-middleware| sig-distributed-middleware| 可正常编译分布式软总线，升级前用例可保持通过 | 
| 【软实时OS】支持分布式数据库| sig-distributed-middleware| sig-distributed-middleware| 对数据库的增删查改操作 | 
| 【软实时OS】支持分布式数据对象| sig-distributed-middleware| sig-distributed-middleware| 对数据库的增删查改操作 | 
| 【软实时OS】支持分布式文件系统| sig-distributed-middleware| sig-distributed-middleware| 分布式文件系统的可用性，文件创建、删除、文件夹创建、删除操作 | 
...
| ----- | ---------------------- | --------------- | ---------------- | ------ |
| 1 | 【特性回合】在RK3568J基于OpenAMP支持openEuler EM+RT-Thread| 基于RK3568J硬件的混合部署，嵌入式linux启动后，可启动RT-Thread，RT-Thread demo输出正常 | 基于RK3568J硬件的混合部署，嵌入式linux启动后，可启动RT-Thread，RT-Thread demo输出正常 |  |
| 2 | 【特性回合】支持外设分区管理|  测试分区管理功能，配置好后，启动linux检查分配给硬实时的硬件是否存在 |  测试分区管理功能，配置好后，启动linux检查分配给硬实时的硬件是否存在 |  |
| 3 | 【特性回合】【软实时OS】支持Jailhouse虚拟化底座|  支持Jailhouse运行，可以通过Jailhouse进行虚拟化操作 | 支持Jailhouse运行，可以通过Jailhouse进行虚拟化操作 |  |
| 4 | 【特性回合】【软实时OS】支持1款X86工控芯片|  编译x86镜像，安装到对应硬件，在硬件上进行基础测试 |  编译x86镜像，安装到对应硬件，在硬件上进行基础测试 |  |
| 5 | 【特性回合】【软实时OS】支持瑞芯微RK3568J|  编译RK3568J镜像，安装到对应硬件，在硬件上进行基础测试 | 编译RK3568J镜像，安装到对应硬件，在硬件上进行基础测试 |  |
| 6 | 【特性回合】【软实时OS】基于RH2288云化PLC硬件，优化RT补丁|  编译RH2288镜像，测试RT补丁性能情况 | 编译RH2288镜像，测试RT补丁性能情况 |  |
| 7 | 【特性回合】【软实时OS】协议优化，提升流式数据和文件传输效率|  测试流数据和文件传输，对比软总线字符和字节传输性能持平，在网络情况不好的情况下更加高效 | 测试流数据和文件传输，对比软总线字符和字节传输性能持平，在网络情况不好的情况下更加高效 |  |
| 8 | 【特性回合】【软实时OS】支持树莓派蓝牙|  启动系统后可查看到蓝牙设备，并被其他蓝牙设备发现 | 启动系统后可查看到蓝牙设备，并被其他蓝牙设备发现 |  |
| 9 | 【特性回合】【软实时OS】支持软总线支持ble发现|  通过编程调用蓝牙发现接口，可发现自动发现对端设备 | 通过编程调用蓝牙发现接口，可发现自动发现对端设备 |  |
| 10 | 【软实时OS】分布式软总线升级到3.2版本|  可正常编译分布式软总线，升级前用例可保持通过 | 可正常编译分布式软总线，升级前用例可保持通过 | 仅在边缘镜像测试，嵌入式此版本不支持 |
...
openEuler 22.03 LTS SP2版本按照社区release-manger团队既定的版本计划，共有5轮测试，按照社区研发模式，所有的需求都在拉分支前已完成合入，因此版本测试采取1+3+1的测试方式，即1轮基本功能保障发布beta版本可以提供给外部开发者，3轮全量保障本次版本发布所有特性(新增&继承)以及系统其他dfx能力，1轮回归。
